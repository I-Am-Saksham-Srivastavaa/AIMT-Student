...
class AimtSupabaseUser extends BaseAuthUser {
  AimtSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> aimtSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AimtSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
